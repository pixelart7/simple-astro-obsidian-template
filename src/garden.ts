...
const GARDEN_PATH = path.join(process.cwd(), 'content');
...
export function generatePathSlug(dir, filename) {
  return [
    dir.split('/').map((d) => slugify(d, { lower: true })).join('/'),
...
export function format(garden) {
  const parsedGarden = garden.map((g) => ({
    ...g,
    gardenDirsAndFile: helperDirsAndFile((g.file.split(path.join(process.cwd(), 'content')).pop().replace(/\.md$/, ''))),
  }))
  ...
  for (const garden of formattedGarden) {
    ...
    $('a').each((_, a) => {
      if (!$(a).attr('href')?.startsWith('http')) {
        postLinks.push({
          slug: String($(a).attr('href')?.replace('/', '')),
          title: $(a).text(),
          abstract: '',
